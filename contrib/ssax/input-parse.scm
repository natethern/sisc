...
(define *read-line-breaks* (map integer->char '(10 12 3)))
(define *return* (ascii->char 12))
...
(define-opt (read-text-line (optional (port (current-input-port))) )
  (if (eof-object? (peek-char port)) (peek-char port)
    (let* ((line
       ...
       (and (eqv? c *return*) (eq? (peek-char port) #\newline)
         (read-char port))			; skip \n that follows \r
       line)))


                                        ; -- procedure+: read-chars->string N [PORT]
...
(define-opt (read-chars->string n (optional (port (current-input-port))) )
  (if (not (positive? n)) ""
...
(define input-parser-error parser-error)
